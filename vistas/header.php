        ...
        <ul class="navbar-nav bg-gradient-primary sidebar sidebar-dark accordion" id="accordionSidebar">
            ...
            <?php if (!empty($menuItems)): ?>
                <?php foreach ($menuItems as $index => $menu): ?>
                    ...
                    <?php
                    // Determinar si es un ítem de menú CON submenú (la forma en que lo definiste)
                    $hasSubmenu = !empty($menu['submenu']);

                    // Determinar si el menú debe estar activo
                    $isActive = $hasSubmenu
                        ? (in_array($currentPage, $menu['pages']) ? 'active' : '')
                        : (($currentPage == ($menu['href'] ?? '')) ? 'active' : '');
                    ?>

                    <li class="nav-item menu <?= $isActive ?>">

                        <?php if ($hasSubmenu): // Caso: Ítem CON submenú (usa data-toggle="collapse") 
                        ?>
                            ...
                            <div id="collapse<?= $index ?>" class="collapse <?= in_array($currentPage, $menu['pages']) ? 'show' : '' ?>" data-parent="#accordionSidebar">
                                <div class="bg-white py-2 collapse-inner rounded divtransparente">
                                    <?php foreach ($menu['submenu'] as $item): ?>
                                        <a class="collapse-item <?= ($currentPage == $item['href']) ? 'active' : '' ?>" href="<?= $item['href'] ?>">
                                            <i class="fas fa-fw <?= $item['icon'] ?>"></i> <?= $item['text'] ?>
                        ...
                        <?php else: // Caso: Ítem SIN submenú (enlace directo) 
                        ?>
